...
{
	program = glCreateProgram(); // create shader program (openGL saves as ref number)
	geoShaders[0] = CreateShader(LoadShader(vertFile), GL_VERTEX_SHADER); // create vertex shader
	geoShaders[1] = CreateShader(LoadShader(geomFile), GL_GEOMETRY_SHADER); // create vertex shader
	geoShaders[2] = CreateShader(LoadShader(fragFile), GL_FRAGMENT_SHADER); // create fragment shader

	for (unsigned int i = 0; i < NUM_SHADERS_GEOMETRY; i++)
	{
...
Shader::~Shader()
{
	...
	for (unsigned int i = 0; i < NUM_SHADERS_GEOMETRY; i++)
	{
		glDetachShader(program, geoShaders[i]); //detach shader from program
		glDeleteShader(geoShaders[i]); //delete the shaders
	}
