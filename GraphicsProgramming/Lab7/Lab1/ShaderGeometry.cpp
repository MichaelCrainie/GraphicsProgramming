...
void ShaderGeometry::init(const std::string& vertFile, const std::string& geomFile, const std::string& fragFile)
{
	...
	shaders[1] = CreateShader(LoadShader(geomFile), GL_GEOMETRY_SHADER); // create vertex shader
...
void ShaderGeometry::Bind()
{
	glUseProgram(program); //installs the program object specified by program as part of rendering state
}

void ShaderGeometry::Update(const Transform& transform, const Camera& camera)
{
...
GLuint ShaderGeometry::CreateShader(const std::string& text, unsigned int type)
{
...
std::string ShaderGeometry::LoadShader(const std::string& fileName)
{
	std::ifstream file;
...
void ShaderGeometry::CheckShaderError(GLuint shader, GLuint flag, bool isProgram, const std::string& errorMessage)
{
	GLint success = 0;
	GLchar error[1024] = { 0 };
